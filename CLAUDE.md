...
## 비용 가드레일 (2026-09-11 C5-A4 — 월 상한 $50)

- **월 상한 $50** (`config.MONTHLY_BUDGET`). 초과 시 프리플라이트가 새 회차를 차단한다.
  상한은 **전체 지출**에 걸리고 실패분(`stage=failed`)도 포함해 센다 — 실패를 예산 밖으로
  빼면 상한이 거짓말이 된다(2026-09-11 실패분 $4.24 = 그달 지출의 27%).
- **예비 구간**: 상한의 마지막 20%($40~$50)에서는 **마감 D-30 이내 질문만** 새 회차를 받는다
  (`MONTHLY_BUDGET_RESERVE_RATIO`·`RESERVE_DEADLINE_DAYS`). 캡으로 떼어 두는 예비비가 아니라
  우선순위 가드다 — 돈은 쓸 수 있고 **대상**만 좁아진다. 막으려는 실패는 실측된 것이다:
  `cpi-jun2026-accel`은 예산이 앞선 질문들에 먼저 쓰여 **첫 예측 없이 만료**했고(처리량 소실
  11.1%), 무예측 만료는 게이트 분자에 0을 더한다.
- **회차당 상한 $4** (`DEFAULT_PIPELINE_BUDGET`). 실측 표준 회차 $1.92~$2.75(n=5, 평균 $2.303).
  내리면 회차가 중간에 죽어 **돈만 쓰고 기록이 안 남는다**.
- **케이던스 주 3건** = 13회차/월 × $2.303 ≈ **$29.9**(상한의 60%). 운영 규칙이며 기계 강제는
  아니다 — 강제하는 것은 위 두 상한뿐.
- **우선순위**: ① 미예측 질문의 r1 ② 마감 D-30 재예측 ③ 정기 재예측. 게이트 문항 수는
  DISTINCT라 **재예측은 문항 수에 0을 더한다** — 예산이 빠듯하면 언제나 r1이 먼저다.
- **게이트 회차 생산자는 로컬 anthropic cli/api 경로 하나뿐이다.** 무인 예약 실행(GitHub
  Actions)은 공식 회차를 생산하지 않는다 — T05가 품질로 은퇴시킨 생산자 조합이 예약 실행에
  그대로 남아 있었고 C5-A4에서 제거했다.
- P0: 질문당 리서치는 서브에이전트 1~4개 이내 (설계서 §09: 질문당 $1.5~4 목표).
