...
- 통계 숫자는 등록된 공식·1차 원천만 허용합니다.
- 통계 파이프라인에서 리포트, 언론, Yahoo, MacroMicro는 아이디어·설명·교차검증 전용입니다.
...
매주 공식 원천을 다시 수집해 원문 영수증과 관측 revision을 먼저 누적하고, 같은 DB에서 Excel·통계·사이트를 재생성합니다. `observation_through`는 마지막 관측일, `knowledge_cutoff`는 빌드가 알 수 있었던 시각입니다. 공식 원천 영수증이 없는 전망 입력은 조용히 대체하지 않고 수치 결합을 차단하며, 아직 공식 빈티지 DB로 이관되지 않은 역사 입력은 연구 후보 상태를 유지합니다.
