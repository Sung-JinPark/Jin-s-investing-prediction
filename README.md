...
`#statistics`에는 1995년과 2023년을 각각 M+0으로 맞춘 20개 비교 그래프가 있습니다. 미국 기술 IPO 수, 인터넷 IPO와 광의 AI 연관 IPO, 엄격한 AI 핵심기업 최소치, 첫날 수익률, 공모가 기준 P/S, 흑자·적자 비중을 함께 보여 상장 공급·가격·이익의 과열을 따로 볼 수 있습니다. 유동성에는 `NASDAQ/M2`와 별도로 Fed Z.1의 가계·비영리단체 현금·입출금예금·정기·저축예금·MMF 합계를 분모로 쓴 `NASDAQ/가계 현금성 자산`도 표시합니다. M2와 이 합계를 더하면 예금이 중복되므로 두 지표는 분리합니다. 데스크톱에서는 두 그래프씩 나란히 보이며, 각 그래프 아래에 현재와 닷컴 시기의 차이를 1~2줄로 설명합니다.
