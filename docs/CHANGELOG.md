...
- **접근-경보(approach alert) 공통 계약**: "경계선에 닿기 전에" 단계가 올라가는
  ...
  6종(m2_nasdaq·nasdaq_per_m2·가계현금성·spx_per_federal_debt·valuation_proxy·
  margin_credit_proxy)은 닷컴 정점. 높을수록 좋은 지표(이익 증가율)에는 달지 않는다.
  임계 %는 표시 규약이며 매매 신호가 아님을 계약 필드와 카드 문구 양쪽에 명시.

- **고점 저항 추세선**: 전 구간 차트에 1968·2000 두 고점을 지나는 빨간 점선 직선을
  추가 — 파란 본선이 이 선에 닿으면 과거 두 정점과 같은 과열 경고 참고 구간.
  고점처럼 추세선도 데이터에서 파생(하드코딩 0)하고, 관측 범위 밖 미래로는 긋지
  않는다. 실측: 저항선 현재 시점 49, 현재 값 35 = 저항선의 71%. 결론·caveat에
  "경고 참고이지 기계적 매매 신호가 아님"을 명시.

- **전 구간 레벨 차트 추가** (`spx_per_federal_debt_full_history`): 5년 창 비교로는
  1968·2000 고점과 현재 위치가 안 보여, 수집 창을 GFDEBTN 원점 1966-01-01로 소급하고
  첫 관측=100 경로를 한 선으로 그렸다. 실측: 1968 고점 103 → 2000 고점 73 → 현재 35 —
  고점이 낮아지는 60년 하락 추세와 2009년 저점 이후 회복이 그대로 드러난다.
  10년 틱·고점 이벤트는 하드코딩이 아니라 데이터에서 파생. 렌더러에 `axis_note`
  지원을 넣어 5년 비교 고정 aria 문구 대신 차트 자체 축 설명을 쓴다.

- 유동성 묶음에 `spx_per_federal_debt` 추가 — SPX/GFDEBTN을 시작=100으로 지수화해
  닷컴·현재를 비교한다 (`nasdaq_per_m2`와 같은 구조). 실측: 닷컴 36개월차 174 vs
  현재 116, 연방부채 $39.1조.
- 분자는 OECD 미국 주가지수 `SPASTT01USM661N`(S&P 계열 월평균, 1957~) — FRED의
  `SP500`은 최근 10년만 제공해 닷컴 구간 비교가 불가능하다. 대용 관계는 caveat에 명시.
  분모 `GFDEBTN`은 재무부 분기말 연방부채(1966~). 두 시리즈 모두 FRED 페이지 실측 확인.
- `statistics-backfill.yml` 신설: 새 시리즈 원장 백필은 FRED_API_KEY가 CI 시크릿에만
  있으므로 브랜치에서 실행해 그 브랜치로 커밋한다 (main 무접촉). workflow_dispatch는
  기본 브랜치에 파일이 있어야 등록되므로(404 실측), 브랜치 단계에서는 파일 안의 요청
  스탬프를 바꿔 푸시하는 것이 실행 요청이다.
