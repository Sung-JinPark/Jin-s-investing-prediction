...
**그래서 단언을 지웠다.** 남은 세 단언이 이미 그 성질을 덮는다 — `S1 > .10` 과
`S3 < -.10` 이 통과하면 폭은 필연적으로 0.20 을 넘는다. 0.25 는 그 위의 5%p 였고,
그 5%p 가 무엇을 막는지는 어디에도 적혀 있지 않았다. **관측값을 바짝 따라가는 절대
문턱을 매일 데이터가 새로 들어오는 자리에 두면, 그 테스트는 성질이 아니라 시장 국면을
지킨다.** 숫자를 0.24 로 내리는 것은 같은 덫을 며칠 뒤로 옮기는 일이라 하지 않았다.

되짚은 표와 사유는 테스트 docstring 에 남겼다 — 다시 넣고 싶은 사람이 **숫자가 아니라
무엇이 깨지는 것을 막는지**부터 적도록.
