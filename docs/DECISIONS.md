...
가 고정한 계약을 건드리지 않았다.
### 2026-09-14 — 해소 3건 + 추정 마감 질문의 무인 예측 차단 (실측 계기: 하마터면 백테스트)

**먼저 해소 3건.** 주간 예약 실행은 예측만 하고 판정은 하지 않아 판정이 밀려 있었다.
게이트 분자를 늘리는 것은 예측이 아니라 해소다. 1·2차 출처가 일치하는 것만 확정했다.

| 질문 | 결과 | 근거 |
|---|---|---|
| `adbe-eps-beat-fq3-2026` | **YES** | non-GAAP EPS $6.13 > 판정 컨센 $6.079 (SEC 8-K EX-99.1) |
| `cpi-aug2026-core-below-24` | **YES** | core CPI-U YoY **2.4%** — 판정기준 `<= 2.4%` 경계값 일치 (BLS 2026-09-11) |
| `orcl-eps-beat-fq1-2027` | **YES** | non-GAAP EPS $1.92 > 판정 컨센 $1.74 (Oracle IR, 발표 2026-09-10) |

adbe 는 **기준 선택이 결과를 뒤집는** 사례로 남는다. 널리 유통되는 $4.84~4.86 컨센서스는
제3기준인데 실제 GAAP EPS 가 $4.62 라 그 기준으로 대조하면 **miss** 다. r2·r3 가 판정
기준을 회사 non-GAAP 으로 못박아 둔 덕에 판정이 흔들리지 않았다.

게이트: 문항 7 → **10**, primary 행 10 → **14**, 행 평균 **0.15986 → 0.1369**.

**그리고 하마터면 백테스트를 할 뻔했다.** `forecast --due --max 3` 의 1순위가
`orcl-eps-beat-fq1-2027` 이었다. 마감은 `2026-09-14`(오늘)인데 **실제 발표는 2026-09-10**
이었고, D-3 재예측 세그먼트는 **2026-09-11 — 이벤트 다음 날**에 열렸다. 우선순위 함수는
"마감이 가장 가깝다"는 이유로 이것을 맨 위에 올렸다. 그대로 돌았다면 **이미 발표된 실적을
예측하는 회차**가 캘리브레이션 표본에 조용히 들어갔다 — 원칙 5(라이브 포워드 only,
백테스트 절대 금지) 위반이고, 예측 파일은 불변이라 **되돌릴 수도 없다**.

기존 가드 `_assert_official_forecast_open` 은 `오늘 > 마감` 일 때만 막는다. 마감이
**확정일**이면 충분하지만 **추정일**이면 실제 이벤트가 먼저 올 수 있어 구멍이 난다.
질문 본문은 스스로 "9/8~9/14 **추정**"이라 적었고 판정기준도 "기한은 발표 예상 상한 —
실제 발표일에 판정"이라 적었는데, **그 사실이 기계가 읽을 수 있는 형태로는 없었다.**

**고친 방식**: 레지스트리에 `deadline_estimated` 를 신설하고(`src_hash` 미포함 — 판정기준이
아니라 "그 날짜를 믿어도 되는가" 메타다), `registry.estimated_deadline_block` 이 마감
**D-7 이내**에서 차단한다. 차단은 **무인 배치(`--due`)에서만** 걸고 해당 질문을 건너뛰며
배치는 계속한다. 사람이 지목한 실행(`forecast <qid>`)은 막지 않는다 — 발표 여부를 확인할
수 있는 것은 사람뿐이고, 확인했다면 D-0 회차가 가장 값진 회차이기 때문이다. 발표일이
확정되면 플래그를 내리면 된다. `deadline` 자체는 판정기준이라 건드리지 않는다.

**같은 모양이 9건 더 있었다** — `asml`(준확정·회사 캘린더 직접 확인 실패) · `tsmc` ·
`msft` · `googl` · `meta` · `amzn` · `aapl` · `nvda-dc-beat-2026nov` · `mu-margin-qoq-fq1fy27`.
전부 표기했다. 오늘 기준 D-7 창에 든 것은 없어 이번 배치는 영향받지 않는다.

**회귀 테스트가 이 결정의 핵심이다.** `test_every_estimated_announcement_date_carries_the_flag`
는 활성 earnings 질문의 **첫 괄호**에 '추정·예상'이 있는데 플래그가 없으면 실패한다 —
다음 사람이 "(장후 추정)" 질문을 하나 더 등록해도 같은 구멍이 다시 열리지 않는다.
반대 방향(`test_the_flag_is_not_sprayed_on_confirmed_dates`)도 함께 고정했다.

**남긴 것**: `mu-margin-qoq-next`(마감 2026-09-29)는 발표일 근거가 레지스트리에 **아무것도
적혀 있지 않다**. 추정인지 확정인지 알 수 없어 임의로 표기하지 않았다 — D-7(2026-09-22)
전에 Micron IR 로 확인이 필요하다.
...
**결정 (사용자 선택: '카드 2개 추가').** 합치지 않고 **나란히** 놓는다. 홈 지표 행에
`다변량 시계열 기준`(V13-VOL `vix25_h21`, 탭과 동일한 `t3_live_card`
게이트)과 `AI 닷컴버블 비교 기준`(`cycle_alignment` 대조축 경과율)을 추가하고, 두 카드
모두 자기 copy 에 **"결합 금지 참고값"** 을 달아 세 확률공간이 한 줄에 나란히 서는
상황을 카드 스스로 고지하게 한다. V13 카드는 기후 기준율을 병기하고
`episode_sample:'thin'`·`reliability:'weak'` 이면 "표본 얇음" 을 남긴다.

**카드 머리말은 번호가 아니라 근거를 말한다 (2026-09-14 추가 지시).** `신호 01~05`
번호를 걷어내고 `가격 시나리오 기준`·`다변량 시계열 기준`·`AI 닷컴버블 비교 기준`·
`변화 감지`·`원장 현황` 으로 바꿨다 — 한 줄에 선 숫자들이 서로 다른 근거에서 나온다는
사실을 머리말이 직접 말하게 해, 번호가 주는 '같은 종류의 신호' 라는 오독을 없앤다.

**닷컴 카드의 % 는 대조축 경과율이다.** 44/59개월 → 75%. 통계 28종은 실업률·물가처럼
방향이 제각각인 **수준값**이라 하나의 '과열도 %' 로 합성하지 않는다 — 합성하면 계약에
없는 지표를 새로 만드는 것이고, 그건 이 결정이 막으려던 바로 그 행위다.
...
## 2026-09-14 — 닷컴 대비 과열도 지수: 이질 지표를 분위수로 접는 사전등록 고정 규칙 (사용자 지시)

**지시.** "과열도를 각각 %로 집계할 수 있잖아. 데이터를 맞추고 그다음에 맞춘 숫자 값에
대한 과열도를 합산하는 거지. 어떻게 계산해야 최적일지 고민해서 진행해."

앞선 결정(같은 날)에서 "28개 이질 지표를 하나의 과열도 % 로 합성하지 않는다" 고 적었다.
그 판단의 근거는 *방향 계약이 없다* 는 것이었는데, 방향은 정의하면 되는 것이고 정의를
지시받았으므로 뒤집는다. 다만 **지어내지 않는 방식**으로만 만든다.

**축: 100% = 닷컴 정점 = 위험.** 읽는 사람이 알고 싶은 것은 "닷컴 정점에 얼마나 가까운가"
다. 그래서 100% 를 닷컴 사이클의 극단에 고정한다.

    higher_is_hotter : 100 * (현재 - 닷컴min) / (닷컴max - 닷컴min)
    lower_is_hotter  : 100 * (닷컴max - 현재) / (닷컴max - 닷컴min)

**방법 (`data/contracts/dotcom_overheat_index_v1.yaml` 에 사전등록).**
1. 닷컴·현재 계열이 **둘 다 있는** 지표만. 계열 라벨을 계약에 명시한다 — 차트 id 만으로는
   `spx_per_federal_debt_full_history` 처럼 시대쌍이 아닌 2계열이 섞인다.
2. 지표별 `direction` 을 **근거 한 줄과 함께** 계약에 적는다 (13종 포함, 15종 제외).
3. 참조창은 **닷컴 전 구간**(1995-01~1999-12). 위상으로 자르지 않는다 — 위험의 기준점은
   1999-2000 의 정점이지 1998 시점의 값이 아니다.
4. 집계는 **부문 중앙값 → 부문 중앙값들의 중앙값**. 평평한 중앙값은 부문별 지표 *개수* 에
   끌려간다(credit 5종 vs valuation 1종) — 개수는 설계가 아니라 어떤 차트가 존재하느냐의
   부산물이라, 그대로 두면 가중치를 우연에 맡기는 셈이다.
5. **적합 없음.** 가중치 학습도 임계 탐색도 없다. 백테스트로 조정하지 않았다.
6. **초과를 자르지 않는다.** >100% 는 닷컴 정점보다 뜨겁다는 뜻이고, 100% 로 클립하면 그
   사실이 사라진다.

**기각한 두 축 (계약의 `rejected_alternatives` 에 사유와 함께 기록).**
- `same_phase_percentile` — 닷컴의 같은 개월차까지 분위수로 재는 방식. 처음 구현했다가
  버렸다: 43개월차 기준이면 참조창이 1995~98 이라 **1999 블로우오프가 아예 빠진다.**
  그러면 100% 가 "1995~98 범위보다 위" 를 뜻하게 되어 정점 위험과 무관해진다. 게다가
  분위수는 100% 에서 포화해 초과 폭을 못 읽는다.
- `cycle_start_anchor` — (현재−닷컴시작)/(닷컴정점−닷컴시작). 시작과 정점이 가까운 지표에서
  분모가 0 에 수렴해 폭발한다. 실측에서 `consumer_credit_growth` **−23,615%**,
  `loan_standards` **−10,200%**. 닷컴 전 구간 범위를 분모로 쓰면 이 불안정이 사라진다.

**실측 (2026-09-14).** 과열도 **61%** (100% = 닷컴 정점). 부문 중앙값 economy 160 ·
rates 69 · credit 61 · valuation 49 · liquidity 33. 포함 13종, 계약 제외 15종,
**닷컴 정점 초과 2종**(`investment_share_gdp` 225%, `corporate_bond_stock` 204%),
닷컴 범위 미만 1종(`consumer_credit_growth` −27%).

**대표값보다 산포가 말이 된다.** 부문이 **33~160%** 로 갈린다 — AI 캐펙스와 기업 부채는
이미 닷컴 정점을 넘었고, 주가/유동성 배율은 아직 정점의 3분의 1 수준이다. 61% 한 숫자만
보면 이게 지워지므로 **카드 copy 가 축의 의미(100% = 닷컴 정점)와 산포를 항상 동반하도록
테스트로 고정**했다.

**한계 (계약에 명기).** ① 지표별 닷컴 참조점 수가 12~44 로 고르지 않다. ② 현재는 33~43개월차
인데 닷컴은 전 구간을 보므로, 사이클 중반에서 정점과 비교한다는 점을 안고 읽어야 한다.
③ 범위 분모가 닷컴의 위기 구간(1998 신용경색 등)을 포함하면 축이 넓어져 현재 값이 낮게
나온다 — `loan_standards`·`bond_spread` 가 이에 해당한다.

**지위.** `probability_space: reference_only` · `model_use: false` ·
`official_forecast_input: false`. 확률이 아니고 다른 확률공간과 산술 결합하지 않는다.

**기록.** `data/contracts/dotcom_overheat_index_v1.yaml`, `src/ai_fc/dotcom_overheat.py`,
`dashboard.py`·`read_model_contract.py`(payload 키), `dashboard.js`(카드),
`test_dotcom_overheat.py`(6종 — 위상 창·방향 반전·얇은 창 제외·부문 집계가 평평한 중앙값과
다름·라이브 계약 정합), `test_dashboard_js_geometry.py`(산포 동반 강제).

## 2026-09-14 — 홈 재설계: 지표 3장 · 최근 변경 제거 · 다음 이벤트 레일 (사용자 지시)

**지시.** "ui를 좀더 깔끔하게. 지금은 말도 많고 … 처럼 넘어가는 글자가 넘어가. 아주
간단명료하게. 최근변경은 빼. 다음 이벤트도 다른 UI로 전면수정."

**같은 날 두 결정을 되돌린다.** 오전에 지표 행을 3장 → 5장으로 늘리고 `UX_AUDIT_260805`
의 '축약' 방향을 명시적으로 거슬렀다. 그 대가가 실제로 나타났다 — 카드가 좁아져 부제가
잘렸고(과열도 카드 131px), 한 줄 → 두 줄 클램프로 막아도 좁은 폭에서는 또 잘렸다.
**정보를 더 넣을수록 읽히는 양은 줄었다.** 축약으로 돌아온다.

**바꾼 것.**
- **지표 5장 → 3장.** 시장 숫자(가격 시나리오·다변량 시계열·AI 닷컴 비교)만 남기고,
  메타였던 `변화 감지`·`원장 현황` 은 카드에서 빼 푸터 한 줄(질문·해소·대기)로 내렸다.
- **카드 내부를 세로 스택으로.** 기존 `auto 1fr` 2칸 배치가 부제 폭을 깎아 잘림을 만드는
  구조였다. 라벨·숫자·한 줄 보조를 세로로 쌓아 부제가 카드 폭을 다 쓴다.
- **경고는 산문 대신 칩.** `표본 얇음`·`정점 초과 2종` 을 부제 뒤에 이어 붙이면 그 부분이
  먼저 잘린다. 별도 칩으로 빼서 길이와 무관하게 남는다.
- **줄 수 제한 자체를 제거.** `nowrap`·`ellipsis`·`-webkit-line-clamp` 모두 뺐다. 말줄임은
  정보 손실을 **숨기기** 때문에 잘린 줄도 모른다.
- **최근 변경 섹션 제거** (지시). `today-columns`·`today-list` 규칙은 홈 전용이었으므로
  죽은 CSS 도 같이 걷어냈다 — 남겨두면 없는 레이아웃이 있는 것처럼 거짓말한다.
- **다음 이벤트 → 레일 UI.** 3칸 리스트(날짜·제목·상태)에서 제목이 말줄임되던 것을,
  auto-fit 카드 레일로 바꿔 제목이 줄바꿈되게 했다. 날짜 옆에 **D-day** 를 넣고 4건으로
  늘렸다(최근 변경이 빠져 폭이 남는다).

**결합 금지 고지를 카드에서 행 주석으로 옮긴다.** 오전 결정은 "페이지 각주가 아니라 카드
스스로 고지" 였다. 카드가 셋뿐이고 한 줄로 나란히 서므로, 반복 태그보다
**"세 숫자는 서로 다른 기준에서 나옵니다 — 더하거나 평균하지 않습니다"** 한 줄이 더
분명하고 읽는 부담이 작다. 축의 의미(`닷컴 정점 100%`)는 카드에 남긴다.

**실측 (1280px · 375px).** 두 폭 모두 가로 잘림 **0**, 페이지 가로 스크롤 0.

**계약 갱신.** `test_ui_contract` 의 `today-columns` → `agenda-rail`,
`test_u1a_five_section_information_architecture_contract` 의 "핵심 지표 5개 · 최근 변경 3
· 다음 이벤트 3" → "핵심 지표 3개 · 다음 이벤트 · 최근 변경 부재 · basis note 존재".
줄 수 고정을 금지하는 회귀 테스트도 CSS 규칙째로 고정했다.


### 같은 날 후속 — 숫자를 평이한 말로, 고지는 한 곳으로 (사용자 지시)

**지시.** "저 %가 직관적으로 뭘 의미하는지 더 쉽게 써. 10%가 뭔지 61%가. 그리고
질문 79·해소 7·as_of·seed·조건부 분포·세 숫자는 서로 다른 기준… 이런 쓸데없는 거 다 없애."

**라벨을 전문용어에서 평이한 말로.** `가격 시나리오`→**연말 주가**,
`다변량 시계열`→**단기 변동성**, `AI 닷컴 비교`→**닷컴 대비 과열도**. 부제는 그 % 가
무엇을 센 비율인지 직접 말한다:

| 숫자 | 부제 |
|---|---|
| 86% | 연말에 전고점 돌파 또는 기준가 유지로 끝난 모의 경로 |
| 10% | 약 한 달 안에 VIX 25 도달 · 평소엔 42% |
| 61% | 100이면 닷컴 버블 정점 · 분야별로 33~160 |

**"서로 다른 기준" 주석줄을 제거한다.** 오전에 카드 반복 태그에서 행 주석으로 옮겼던
그 줄이다. 라벨이 평이한 말로 *무엇을 재는지* 직접 말하게 되면서 — 연말 주가·단기
변동성·닷컴 과열도가 같은 종류로 읽힐 여지가 사라졌다 — 주석이 설명이 아니라 잡음이 됐다.
**규약을 포기한 게 아니라 설계로 흡수했다.**

**홈 자체 푸터(`today-context`) 제거.** 질문·해소·대기 수, as_of·seed·경로 수, 조건부
분포 고지를 모두 뺐다. 다만 **투자자문 고지는 규약상 남아야 한다**(CLAUDE.md — P3 게이트
전 참고 의견 지위 유지). 전역 `site-footer` 가 이미
"정보 제공 목적 · 투자 자문이나 매매 권유가 아닙니다 · 과거 기록은 미래 성과를 보장하지
않습니다" 를 싣고 있었는데 홈에서만 `display:none` 으로 숨겨져 있었다. 그 규칙을 없애
**고지를 사이트 전체에서 한 곳으로 모았다** — 홈에서 지우는 대신 정본 자리로 옮긴 셈이다.
...
## 2026-09-17 — 12-10 홈 "다음 이벤트": 레일 → 종류 인덱스 보드

**위임 근거**: 사용자 지시 "다음 이벤트에서 너무 카드가 길어서 우측 반이 휑해. 그걸 조정하거나
다른 좋은 아이디어 ui있는지 봐바 뭐 좀 더 디지털 캘린더나 아날로그 캘린더 모양으로 아래를
채운다거나 등 검토 후 적용해봐" (2026-09-17), 이어서 "근거 뭐 이런건 없애고 그 아래 확정 12건
추정 34건 이런거 없애고 깔끔한 ui로".

**불만은 둘이었다** — ① 세로로 길다 ② 우측 절반이 휑하다. 원인은 실측된다:
`.event-rail-item a` 의 `54px · minmax(56px,1fr) · minmax(0,2.2fr)` 에서 682px 짜리 제목 칸에
들어가는 글자가 51~111px(채움률 16%)이고, 가운데 트랙의 막대는 3~11% 였다. 3열 중 2열이
데이터를 거의 담지 않았다.

| # | 항목 | 결정 | 근거 |
|---|---|---|---|
| 12-10a | **달력 격자 기각** | 사용자가 힌트로 준 월 격자를 쓰지 않는다 | 실데이터가 11개월 중 10개월이 '4건 / 서로 다른 4일' 균일 격자다. 7×N 셀의 **85~90% 가 빈칸**이 되고 세로는 데스크톱 +29px / 400px +70px 늘어난다 — '빈 트랙 하나'를 '빈 칸 45개'로 바꾸는 거래이고 불만 ① 을 악화시킨다. 달력에서 값어치 있는 부분(월/일/요일)은 카드 왼쪽 tear-off 조각으로 흡수했다 |
| 12-10b | 3열 카드 보드 | 세로로 4행 쌓던 것을 3열 × 2행으로 접는다 | 682px 빈 칸 하나를 361px 카드 3장으로 쪼개면 본문 폭이 287px 로 줄어 같은 글자가 칸을 채운다(채움률 16% → 40~70%). 카드 수는 **6** — 3 으로도 2 로도 나누어떨어져 어느 중단점에서도 마지막 줄에 고아 카드가 생기지 않는다 |
| 12-10c | 진행 막대 폐기 | 거리 표현을 막대에서 D-day 숫자로 되돌린다 | 4건짜리 축 위에서 막대는 트랙의 3~11% 였다. 거리는 `D-13` 이 이미 말하고, 막대는 그 말을 97% 의 회색으로 반복했다 |
| 12-10d | **선정은 종류 인덱스 우선** | 시간순이 아니라 종류별 첫 회차를 먼저 집고 남는 자리만 시간순으로 채운다(`evPick`) | 순수 시간순은 계절을 탄다 — 2026-12-01 창을 실제로 돌리면 시간순 6건이 nfp·fomc·cpi·gdp·nfp·cpi 로 **실적이 한 장도 안 나온다**. 사용자의 EXIT 트리거가 CPI·고용·FOMC·실적이므로 "다음 실적 언제"는 데이터 밀도와 무관하게 답해야 한다. 정렬은 마지막에 날짜순으로 되돌려 읽는 순서 = 시간 순서를 지킨다 |
| 12-10e | 추정 판정은 `status` 로만 | `time_et` 공백을 프록시로 쓰지 않는다 | 실측상 `time_et` 공백 34건 = estimated 34건으로 정확히 겹쳐 '최적화'하고 싶어지는 자리다. 묶는 순간 **시각 없는 확정 일정이 하나 들어오면 화면이 거짓말한다**. 미지의 status 는 추정 쪽으로 떨어뜨리고, 묶음 행(10/28 확정 FOMC + 추정 실적)은 원본을 다시 훑어 추정이 하나라도 있으면 추정으로 그린다 — 추정을 확정처럼 그리는 쪽으로는 틀리지 않는다 |
| 12-10f | **표식은 예외에만** | 확정에는 아무것도 붙이지 않고 추정에만 파선 테두리 + `추정` 알약 | 6장 중 4장이 확정이라 전부 붙이면 알약이 배경이 되고 정작 도드라져야 할 추정이 묻힌다. 화면 낭독기에는 tear-off 의 `sr-only` 가 카드마다 확정·추정을 그대로 읽어 준다 — 비시각 경로에서는 6장 모두 명시된다 |
| 12-10g | 정의·출처·건수는 한 곳으로 | 홈에서 주석 문단·규모 칩·행별 근거 링크를 걷어낸다 | 사용자 요청("깔끔한 ui")이고, **지운 게 아니라 모은 것**이다: `calendar_sources.yaml` 의 `confirmed_definition`/`estimated_definition` 과 `probability_linkage: forbidden` 고지는 `#future` 의 전체 달력이 싣고, 행별 '공식 근거'와 `확정 12 · 추정 34` 요약 칩도 거기 있다. 홈의 '전체 일정' 링크가 그곳을 가리킨다. 계약은 **정책**이지 특정 화면 문구를 요구하지 않는다 |
| 12-10h | '전체 일정' 링크 정정 | `#records/journal` → `#future` | 전자는 라우트 정의상 **변경 일지**다. 전체 일정은 `#future` 의 `market-event-calendar` 다 — 링크가 이름과 다른 곳으로 가고 있었다 |
| 12-10i | 무범위 `.event-track` 제거 | `.event-track{position:relative;height:3px}`(12-9 때 홈 레일용으로 넣은 전역 규칙)을 걷어낸다 | `.market-event-calendar .event-track` 는 height 를 선언하지 않아 이 전역 규칙이 그대로 걸린다 — `min-height:138px` 카드들이 3px 상자에 갇힌다. **이름 재사용이 만든 충돌**이고, 새 표면은 `.ev-*` 네임스페이스를 써서 같은 실수를 반복하지 않는다. 해당 패널이 현재 렌더되지 않아 육안 확인은 못 했다(잠재 결함) |

**실측 (전 → 후)**

| 폭 | 세로 | 노출 건수 |
|---|---|---|
| 1440px | 267 → **182px** (−32%) | 4 → 6 |
| 700px | — → 218px (2열 × 3행) | 6 |
| 400px | 403 → **185px** (−54%) | 4 → 4 |

400px 에서 5·6번 카드를 감추는 것(`nth-child(n+5)`)이 세로 이득의 핵심이다 — 2열 3행이 되면
이득이 사라진다. 지금 화면도 4건이라 모바일 노출은 줄지 않는다.

**설계 경로.** 네 안(월 격자 / 목록+12개월 리본 / 종류별 레인 / 자유 재발명)을 병렬로 세우고
적대적으로 심사했다. 최고점은 레인(38/60)이었으나 채택하지 않았다 — 970px 플롯 위 잉크가
레인당 5.6% 라 "아래를 채운다"는 요청의 정반대이고, 제목·시각·링크가 홈에서 사라지는 기능
회귀가 붙는다. 채택안은 리본 안의 카드층을 뼈대로 삼고 자유안의 종류 인덱싱을 이식한 것이다.
