...
1. 닷컴·현재 계열이 **둘 다 있는** 지표만 쓴다. 계열 라벨을 계약에 명시한다(차트 id 만으로는
   `spx_per_federal_debt_full_history` 처럼 시대쌍이 아닌 2계열이 섞인다).
2. 지표별 `direction`(higher/lower_is_hotter)을 **근거 한 줄과 함께** 계약에 적는다.
3. 과열도_i = 현재 최신값이 **닷컴의 같은 사이클 시점까지** 분포에서 갖는 midrank 분위수.
   전 구간이 아니라 위상을 맞추는 이유: 43개월차 값을 닷컴 59개월차 정점과 비교하면
   체계적으로 낮게 나온다(matched-month 관례).
...
5. **적합 없음.** 가중치 학습도 임계 탐색도 하지 않는다 — 전부 사전등록 고정 규칙이며,
   CLAUDE.md 가 허용하는 결합 형태(중앙값)에 머문다. 백테스트로 조정하지 않았다.

**실측 (2026-09-14).** 과열도 **77%**, 부문 중앙값 valuation 100 · economy 100 ·
liquidity 77 · credit 58 · rates 8. 포함 13종, 계약 제외 15종, 닷컴 창 최대 초과 4종.

**가장 중요한 결과는 대표값이 아니라 산포다.** 부문이 **8~100%** 로 갈린다 — 자산·밸류에이션·
실물 캐펙스는 닷컴 같은 시점보다 뜨겁고, 금리·스프레드 여건은 훨씬 차갑다. 77% 한 숫자만
보면 이 양극화가 지워지므로 **카드 copy 가 산포를 항상 동반하도록 테스트로 고정**했다.

**한계 (계약에 명기).** ① 분위수는 100% 에서 포화해 '조금 위' 와 '크게 위' 를 구분하지
못한다 — 창 최대 초과 지표 수를 따로 보고한다. ② 지표별 닷컴 참조점 수가 12~44 로 고르지
않다. ③ 현재 최신 period 가 33~43개월차로 지표마다 달라 위상이 완전히 같지는 않다.
