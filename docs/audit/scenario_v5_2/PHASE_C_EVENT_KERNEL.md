...
Only one eligible event is available for a direct historical event-return map. Under the n<30 rule, the hard event kernel is `REFERENCE_ONLY_INSUFFICIENT_N`; it contributes no direct price jump. The August 7 Nasdaq return is already in the 26,690.62 anchor. Future event jump = 0, event-return coefficient = 0, equality-with-zero-event-reaction gate = `True`.
