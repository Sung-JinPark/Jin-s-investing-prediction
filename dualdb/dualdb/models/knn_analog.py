"""k-NN 아날로그 (Q15) — 닷컴 상태공간에서 현재 AI 국면의 최근접 이웃 탐색.

절차:
1. 닷컴 시대 ^IXIC 각 월말(마지막 거래일) 시점의 5차원 상태벡터
   [vol_20d, vol_60d, drawdown, dist_200dma, rsi_14] 수집 (derived_daily).
2. 닷컴 표본만으로 z-표준화 모수(평균·표준편차) 산출 — AI 데이터가 모수에
   섞이면 누출이므로 금지. 현재 AI 최신 벡터를 같은 모수로 변환.
3. 유클리드 거리 k=5 최근접 닷컴 시점 선택. 이웃 간 최소 90일 간격 강제
   (같은 국면 중복 방지 — 거리 오름차순 greedy).
4. 각 이웃의 이후 1/3/6/12개월(21/63/126/252 거래일) ^IXIC 수익률 분포 산출.
   이웃+지평이 닷컴 창(2003-12-31)을 넘으면 해당 지평은 None — 외삽 금지.

한계 (정직성 고지 — §10):
- **표본 n=1 사이클**: 이웃이 전부 단일 닷컴 사이클에서 나온다. 통계적
  검정력 없음 — 확률이 아니라 아날로그 참조(base rate 입력)로만 사용.
- **상태벡터 불완전**: 가격 파생 5차원만 사용. 심리(AAII)·크레딧(HY 스프레드)·
  밸류에이션 차원은 일간 정렬 가능한 데이터 부재로 결측 — "비슷한 가격 국면"이지
  "비슷한 시장 전체 상태"가 아니다.
- **자기상관**: 동일 사이클 내 이웃들은 서로 독립이 아니다. 90일 간격으로
  완화할 뿐 독립 표본이 되지 않는다.
- **지수 구성 변화**: ^IXIC 구성종목이 두 시대 간 전면 교체됨 (지수 수준의
  생존편향 유사 효과) — 시대 간 직접 비교에 구조적 왜곡 가능.
기록: model_run 테이블에만 INSERT (model='knn_analog') — 원천 계층 무접촉.
"""

from __future__ import annotations

import json
import sqlite3
from bisect import bisect_left
from datetime import date, datetime

import numpy as np

from ..derive.daily import ERA_WINDOWS

SERIES = "^IXIC"
FEATURES = ("vol_20d", "vol_60d", "drawdown", "dist_200dma", "rsi_14")
HORIZONS_TD = {"fwd_1m": 21, "fwd_3m": 63, "fwd_6m": 126, "fwd_12m": 252}
K_DEFAULT = 5
MIN_GAP_DAYS = 90

CAVEATS = [
    "표본 n=1 사이클(닷컴) — 통계적 검정력 없음, base rate 참조용 참고 의견",
    "상태벡터는 가격 파생 5차원만 — 심리(AAII)·크레딧(HY)·밸류에이션 차원 결측",
    "피처 간 상호 상관 미백색화(vol20↔vol60, drawdown↔dist_200dma) — 유클리드 거리가"
    " 잠재요인을 중복 계수, 유효 차원 < 5 (RE-AUDIT R-4; Mahalanobis/PCA 백색화는"
    " 이웃 집합을 실질 변경하므로 별도 검증 라운드에서 결정)",
    "동일 사이클 내 이웃은 자기상관 — 90일 간격 강제로 완화하나 독립 표본 아님",
    "이웃+지평이 닷컴 창(2003-12-31)을 넘으면 None — 외삽 금지",
    "^IXIC 구성종목 전면 교체 — 지수 수준 생존편향 유사 왜곡 가능",
    "전방수익률 표는 **질문 매핑 확률이 아님 — 참조 base rate** (F1/F3 지평과 겹치므로"
    " 프롬프트 주입 시 준-앵커 주의, R-4)",
]


def _dotcom_month_end_vectors(conn: sqlite3.Connection
                              ) -> tuple[list[str], np.ndarray]:
    """닷컴 시대 각 월말(마지막 거래일)의 5차원 상태벡터 — 전 피처 non-NULL만."""
    cols = ", ".join(f"d.{f}" for f in FEATURES)
    cond = " AND ".join(f"d.{f} IS NOT NULL" for f in FEATURES)
    rows = conn.execute(
        f"""SELECT d.date, {cols} FROM derived_daily d
            JOIN (SELECT MAX(date) md FROM derived_daily
                  WHERE series=? AND era_id='dotcom' GROUP BY substr(date,1,7)) me
              ON d.date = me.md
            WHERE d.series=? AND d.era_id='dotcom' AND {cond}
            ORDER BY d.date""", (SERIES, SERIES)).fetchall()
    dates = [r["date"] for r in rows]
    X = np.array([[r[f] for f in FEATURES] for r in rows], dtype=float)
    return dates, X


def _ai_latest_vector(conn: sqlite3.Connection) -> tuple[str, np.ndarray]:
    """AI 시대 최신 시점의 상태벡터 (전 피처 non-NULL인 가장 최근 행)."""
    cols = ", ".join(FEATURES)
    cond = " AND ".join(f"{f} IS NOT NULL" for f in FEATURES)
    row = conn.execute(
        f"""SELECT date, {cols} FROM derived_daily
            WHERE series=? AND era_id='ai' AND {cond}
            ORDER BY date DESC LIMIT 1""", (SERIES,)).fetchone()
    if row is None:
        raise ValueError("AI 시대 완전 상태벡터 없음 — derive 후 실행")
    return row["date"], np.array([row[f] for f in FEATURES], dtype=float)


def _zfit(X: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """닷컴 표본만으로 z 모수 산출 (누출 방지). 상수 피처는 sd=1 가드."""
    mu = X.mean(axis=0)
    sd = X.std(axis=0, ddof=1)
    sd = np.where(sd > 0, sd, 1.0)
    return mu, sd


def _greedy_select(dates: list[str], dist: np.ndarray, k: int,
                   min_gap_days: int) -> list[int]:
    """거리 오름차순 greedy — 기선택 이웃과 min_gap_days 미만이면 건너뜀."""
    chosen: list[int] = []
    for idx in np.argsort(dist, kind="stable"):
        d = date.fromisoformat(dates[idx])
        if all(abs((d - date.fromisoformat(dates[j])).days) >= min_gap_days
               for j in chosen):
            chosen.append(int(idx))
        if len(chosen) == k:
            break
    return chosen


def _forward_returns(price_dates: list[str], closes: np.ndarray,
                     neighbor_date: str) -> dict[str, float | None]:
    """이웃 시점 이후 21/63/126/252 거래일 수익률 — 배열 범위 밖이면 None.

    price_dates/closes는 닷컴 창(2003-12-31)까지로 잘려 들어와야 한다 —
    범위 초과가 곧 외삽 금지 None이 되는 구조.
    """
    i = bisect_left(price_dates, neighbor_date)
    if i >= len(price_dates) or price_dates[i] != neighbor_date:
        raise ValueError(f"price_daily에 이웃 시점 {neighbor_date} 없음 — DB 불일치")
    c0 = closes[i]
    out: dict[str, float | None] = {}
    for name, td in HORIZONS_TD.items():
        j = i + td
        out[name] = round(float(closes[j] / c0 - 1), 4) if j < len(closes) else None
    return out


def _dotcom_prices(conn: sqlite3.Connection) -> tuple[list[str], np.ndarray]:
    w0, w1 = ERA_WINDOWS["dotcom"]
    rows = conn.execute(
        "SELECT date, close FROM price_daily WHERE series=? AND date BETWEEN ? AND ?"
        " ORDER BY date", (SERIES, w0, w1)).fetchall()
    return [r["date"] for r in rows], np.array([r["close"] for r in rows], dtype=float)


def run(conn: sqlite3.Connection, k: int = K_DEFAULT,
        min_gap_days: int = MIN_GAP_DAYS, record: bool = True) -> dict:
    """k-NN 아날로그 실행. record=True면 model_run에 1행 기록 (twins.run과 동일 API).

    반환 dict는 render_md 입력 겸용 — run_id는 record=True일 때만 포함.
    """
    dc_dates, X = _dotcom_month_end_vectors(conn)
    if len(dc_dates) < k:
        raise ValueError(f"닷컴 월말 표본 {len(dc_dates)}개 < k={k} — derive 후 실행")
    asof, v = _ai_latest_vector(conn)

    mu, sd = _zfit(X)                      # 닷컴 표본만 — 누출 방지
    Z = (X - mu) / sd
    z = (v - mu) / sd
    dist = np.sqrt(((Z - z) ** 2).sum(axis=1))
    chosen = _greedy_select(dc_dates, dist, k, min_gap_days)

    price_dates, closes = _dotcom_prices(conn)
    neighbors = []
    for idx in chosen:
        rec: dict = {"date": dc_dates[idx], "distance": round(float(dist[idx]), 4)}
        rec.update(_forward_returns(price_dates, closes, dc_dates[idx]))
        neighbors.append(rec)

    # 각 지평의 중앙값에 유효 표본수 n 병기 — None(외삽 금지) 제외 후 남는 표본
    median_fwd: dict[str, dict] = {}
    for h in HORIZONS_TD:
        vals = [n[h] for n in neighbors if n[h] is not None]
        median_fwd[h] = {
            "median": round(float(np.median(vals)), 4) if vals else None,
            "n": len(vals),
        }

    result = {
        "asof": asof,
        "n_dotcom_samples": len(dc_dates),
        "feature_vector": {
            "raw": {f: round(float(v[i]), 6) for i, f in enumerate(FEATURES)},
            "z": {f: round(float(z[i]), 3) for i, f in enumerate(FEATURES)},
        },
        "neighbors": neighbors,
        "median_fwd": median_fwd,
        "caveats": list(CAVEATS),
    }
    if len(chosen) < k:
        result["warning"] = (
            f"이웃 {len(chosen)}/{k}개만 선택 — min_gap_days={min_gap_days} 간격 "
            "제약으로 k 미달 (중앙값 표본 축소, 해석 주의)")
    params = {
        "k": k, "min_gap_days": min_gap_days, "series": SERIES,
        "features": list(FEATURES), "horizons_trading_days": HORIZONS_TD,
        "standardize": "dotcom-only z (누출 방지)",
        "z_mu": [round(float(x), 6) for x in mu],
        "z_sd": [round(float(x), 6) for x in sd],
    }
    if record:
        cur = conn.execute(
            "INSERT INTO model_run (model, asof, params_json, output_json, created_at)"
            " VALUES (?,?,?,?,?)",
            ("knn_analog", asof, json.dumps(params, ensure_ascii=False),
             json.dumps(result, ensure_ascii=False),
             datetime.now().isoformat(timespec="seconds")))
        conn.commit()
        result["run_id"] = cur.lastrowid
    return result


def _pct(x: float | None) -> str:
    return "—" if x is None else f"{x:+.1%}"


def render_md(result: dict) -> str:
    """run() 결과 dict → 마크다운 (한계 고지 포함 — 없으면 출력 무효)."""
    fv = result["feature_vector"]
    lines = [
        f"# k-NN 아날로그 (Q15) — asof {result['asof']}",
        "> 참고 의견 (P3 게이트 전) · 표본 n=1 사이클 — 확률 아님, 아날로그 참조",
        "",
        f"## 현재 상태벡터 (^IXIC AI 시대, 닷컴 월말 {result['n_dotcom_samples']}개 표본 기준 z)",
        "| feature | raw | z |",
        "|---|---|---|",
    ]
    for f in FEATURES:
        lines.append(f"| {f} | {fv['raw'][f]:.4f} | {fv['z'][f]:+.2f} |")
    k = len(result["neighbors"])
    lines += [
        "",
        f"## 최근접 닷컴 이웃 k={k} (유클리드, 이웃 간 90일+ 간격)",
        "| # | 닷컴 시점 | 거리 | +1m | +3m | +6m | +12m |",
        "|---|---|---|---|---|---|---|",
    ]
    for i, n in enumerate(result["neighbors"], 1):
        lines.append(
            f"| {i} | {n['date']} | {n['distance']:.3f} | {_pct(n['fwd_1m'])} |"
            f" {_pct(n['fwd_3m'])} | {_pct(n['fwd_6m'])} | {_pct(n['fwd_12m'])} |")
    m = result["median_fwd"]
    lines.append(
        "| **중앙값 (n)** | | | "
        + " | ".join(f"**{_pct(m[h]['median'])}** (n={m[h]['n']})"
                     for h in ("fwd_1m", "fwd_3m", "fwd_6m", "fwd_12m"))
        + " |")
    if result.get("warning"):
        lines += ["", f"> ⚠ {result['warning']}"]
    lines += ["", "## 한계 (정직성 고지)"]
    lines += [f"- {c}" for c in result["caveats"]]
    return "\n".join(lines) + "\n"
