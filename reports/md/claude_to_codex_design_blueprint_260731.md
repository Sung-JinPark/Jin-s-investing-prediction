...
## Codex 정밀 대조·실행 결과 (2026-07-31)

청사진 문구와 2026-07-31 현재 코드를 파일·테스트 단위로 대조했다. 결론은 일부 “미구현”
진단이 작성 시점보다 뒤처져 있었고, 그대로 재구현하면 오히려 데이터 정직성 규칙을 해칠
수 있다는 것이다.

| WS | 대조 결과 | 이번 실행 |
|---|---|---|
| WS-1 | 가격형 초안·다회차 채점·`research_status` 자동 태깅은 이미 구현. 실제 공백은 macro/earnings와 resolve 경과일 정렬 | 두 출처 구조화 관측 JSON, registry 문언 기반 수치 규칙, 불일치 `held`, 원장 무기록, 경과일 우선 큐 구현 |
| WS-2 | analog/factor/regime context는 이미 digest에 배선. `event`만 미배선 | 현 AI M+N ±6개월 과거 사건을 `event_context`로 추가하고 “서사·매핑 확률/트윈 표본 아님” 라벨로 digest 주입 |
| WS-3 | **다중 시대 풀은 이미 구현 완료**(`pool-only z`, 닷컴 외 이웃 테스트·CHANGELOG #15) | 재구현하지 않음. CAPE/HY는 교차 시대 커버가 없어 차원 추가 시 풀 전멸하므로 청사진 제안을 기각하고 레짐 컨텍스트 유지 |
| WS-4 | AAII 수동 파일 미제공, fundamentals 공백. `cycle_compare`는 소비자 0인 long-format 잔존 | 가짜 데이터·빈 파서 생성 없이 보류 |
| WS-5/6 | 게이트 전 표시·섀도 배관만 존재 | 활성화하지 않음 |
| WS-7 | read-model 필수 키 유지 중 | `due[].overdue_days`만 additive 추가, 기존 키 삭제·개명 없음 |
| WS-8 | schedule·stale·divergence 표시 구현. 이벤트 트리거 자동 실행은 비용 규율상 미구현 | 자동 예측 팬아웃 없음 |

판정 관측 JSON은 연산자·임계값을 가질 수 없다. 그 값은 오직 기존 registry 판정 문언에서
읽어 evidence가 판정기준을 우회하지 못하게 했다. 두 출처가 다르면 평균·반올림·보간 없이
`held`다. 최종 원장 기록은 기존 인간 확인 경로만 유지한다.
