...
공식 지급배당 표의 주당 금액은 1.121, 1.151, 1.181, 1.241, 1.346달러다. Yahoo
ex-date 이벤트의 연도 합계도 같은 증가 방향을 보였지만 지급연도와 ex-date 기준 차이 때문에
두 합계의 정확한 일치는 주장하지 않는다.
...
### 2.3 네 개의 12개월 조건부 경로 — O v2 정정

> **2026-08-04 O 경로 v2:** 고정 `o_offset`을 제거했다. O는 NASDAQ 국면 beta에
> 156주 주간 회귀로 측정한 금리·신용 민감도와 사전 등록한 Δ10Y·ΔHY 경로만 더한다.
> 금리 민감도는 +100bp당 -8.372%(10–90% CI -10.646~-6.259%, n=156), 신용 민감도는
> +100bp당 -6.062%(CI -7.841~-4.362%, n=156)로 이번 스냅샷에서는 두 게이트가 모두
> 열렸다. CI가 0을 가로지르거나 n<156이면 해당 항은 0으로 처리한다.

| 충격 경로 | M+12 NASDAQ | M+12 Bitcoin | M+12 O | 핵심 조건 |
|---|---:|---:|---:|---|
| 동반 디레버리징 | 82.0 | 70.2 | 89.1 | 금리는 하락하지만 신용경색이 더 빠름 |
| AI 조정 후 완화·순환 | 91.0 | 130.6 | 111.3 | 장기금리 하락·크레딧 진정 |
| 소프트랜딩·자산 순환 | 112.0 | 132.1 | 96.6 | 금리 소폭 상승·신용 안정 |
| 금리가 안 내려오는 붕괴 | 82.0 | 70.2 | 76.0 | NASDAQ 경로는 디레버리징과 같고 금리·신용은 악화 |
...
beta를 적용한다. Bitcoin은 기존 명시 offset을 유지하지만, O는 고정 offset 없이 사전 등록한
거시 경로와 유의성 게이트를 통과한 측정 민감도만 적용한다. beta 하한은 강제하지 않으며 절대값
...
src/ai_fc/cross_asset.py
  ...
  ├─ O 금리·신용 156주 회귀 + 유의성 gate
  ├─ 4개 조건부 12개월 전이 경로 + 10–90% band
...
data/realty_income/
  ├─ dividends.csv (append-only)
  └─ rate_sensitivity_latest.json + archive/
data/rate_events/
  ├─ registry.yaml (사전 등록 6건)
  └─ event_study_latest.json + archive/
          │
          ▼
