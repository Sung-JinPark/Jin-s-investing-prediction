...
function statisticsChartSvg(chart){
  ...
  const maxPeriod=Math.max(1,...points.map(row=>Number(row.period)||0));
  ...
  const xTicks=[0,12,24,36,maxPeriod].filter((value,index,array)=>value<=maxPeriod&&array.indexOf(value)===index).sort((a,b)=>a-b);
  const line=values=>values.map((row,index)=>`${index?'L':'M'}${X(row.period).toFixed(1)},${Y(row.value).toFixed(1)}`).join(' ');
  return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(chart.title)} 닷컴과 현재 비교 차트">
...
function renderStatistics(){
  ...
  root.appendChild(el(`<div class="page-heading statistics-heading"><div><p class="eyebrow">STATISTICS · DOTCOM VS NOW</p><h1>닷컴과 지금, 숫자로 나란히 보기</h1><p class="page-lede">유동성·금리·기업가치·신용을 같은 경과월로 맞춰 비교합니다. 과거와 닮았는지 확인하는 참고 통계이며 예측확률이나 투자 신호가 아닙니다.</p></div></div>`));
  if(stats.status!=='ok'){
  ...
  root.appendChild(el(`<section class="statistics-summary" aria-label="통계 비교 기준"><article><span>비교 시작</span><strong>닷컴 ${esc(alignment.dotcom_start)}<br>현재 ${esc(alignment.current_start)}</strong></article><article><span>같은 시간축</span><strong>M+0 ~ M+${num(alignment.comparison_months||0)}</strong></article><article><span>그래프</span><strong>${num(charts.length)}개</strong></article><article><span>데이터 기준</span><strong>${esc(stats.as_of||'—')}</strong></article></section>`));
  root.appendChild(el(`<section class="statistics-notice" role="note"><div><span>주 1회 DB 확인</span><strong>매주 공개 원천을 다시 확인합니다</strong></div><p>일간·월간·분기 통계의 원래 발표 주기는 바꾸지 않습니다. 과거 값은 최신 릴리스로 재구성한 역사이며 당시 시점의 원본 빈티지가 아닙니다.</p></section>`));
  const categories=[['all','전체'],['liquidity','유동성'],['rates','금리'],['valuation','기업가치'],['credit','신용']];
...
const FLOW_LAB_COPY={
  ...
  'cross-asset':['자산 전이 · Dotcom Counterfactual','닷컴 붕괴 뒤 NDX·BTC·O는 어떻게 달라졌을까','2001-03~2006-03 NASDAQ·O 실측과 BTC 반사실 민감도를 같은 기준점에서 비교합니다.'],
...
function renderScenarioV52(candidate,initialState={}){
  ...
  root.appendChild(el(`<div class="page-heading"><div><p class="eyebrow" id="v52-page-eyebrow">미래 탐색 · 연구 후보</p><h1 id="v52-page-title">Scenario V5.2 조건부 경로</h1><p class="page-lede" id="v52-page-lede">서로 다른 데이터 군집에서 만든 세 경로를 같은 로그 스케일로 비교합니다. 보정된 상승·중립·하락 발생확률이 아닙니다.</p></div></div>`));
  root.appendChild(el(scenarioModelModeNav('research',candidate)));
  root.appendChild(el(`<section class="scenario-v52-risk-banner" role="status" aria-label="연구 후보 승격 상태"><div><span>비승격 연구 후보</span><strong>적격 사건 ${num(hard.eligible_historical_event_count||0)}/${num(hard.preferred_minimum||60)} · ${esc(candidate.status)} · 보정되지 않음</strong></div><p>champion이 아니며 기본 화면에 사용하지 않습니다. band calibration ${num(promotionGates.band_calibration?.observations||0)}/${num(promotionGates.band_calibration?.minimum||60)} · 구분성 ${esc(distinctness.operational_mode||'report_only')} ${num(distinctness.threshold_calibration?.observations||0)}/${num(distinctness.threshold_calibration?.minimum||30)} · 승인 run_id ${esc(promotionGates.human_approval?.approval_run_id||'없음')}.</p></section>`));
  const outlook=el(`<div id="lab-future" role="tabpanel" aria-labelledby="lab-tab-future">
    <section class="scenario-v52-overview" aria-label="전망 기준 요약"><div><span>시장 기준일</span><strong>${esc(candidate.as_of.slice(0,10))}</strong></div><div><span>현재 지수</span><strong>${num(Math.round(anchor))}</strong></div><div><span>모의 경로</span><strong>${num(candidate.model.path_count)}개</strong></div><div><span>상태</span><strong>비승격 연구 후보</strong></div></section>
    ...
    <section class="scenario-v52-outcomes" aria-labelledby="scenario-v52-outcomes-title"><div class="scenario-v52-section-title"><p class="eyebrow">연구 후보 결과 비율</p><h2 id="scenario-v52-outcomes-title">그래프에서 바로 확인할 네 가지</h2><p>보정되지 않은 모의 경로 비율을 정수로 반올림했습니다. 코호트 가중치나 공식 사건확률과 결합하지 않습니다.</p></div><div><article><span>2026 연말</span><strong>${pct(full.terminal_above_anchor_2026)}</strong><p>현재 지수보다 높게 끝난 모의 경로 비율</p></article><article><span>2027 연말</span><strong>${pct(full.terminal_above_anchor_2027)}</strong><p>현재 지수보다 높게 끝난 모의 경로 비율</p></article><article><span>2026 최고치</span><strong>${pct(full.new_ath_by_2026)}</strong><p>연말까지 새 최고치를 한 번이라도 넘은 비율</p></article><article><span>−10% 조정</span><strong>${pct(full.first_touch_minus_10_by_october_end)}</strong><p>10월 말까지 조정선을 한 번이라도 만난 비율</p></article></div></section>
    <section class="scenario-v52-insights" aria-labelledby="scenario-v52-insights-title"><div class="scenario-v52-section-title"><p class="eyebrow">숫자의 뜻</p><h2 id="scenario-v52-insights-title">복잡한 지표를 이렇게 읽으세요</h2></div><div>
      <article><span>고용과 금리</span><strong>${pp(terminalAttr.labor_growth_risk_effect)} / ${pp(terminalAttr.policy_relief_effect)}</strong><p>고용 약화는 성장 위험을 높였고, 금리 부담 완화는 거의 같은 크기로 이를 상쇄했습니다.</p></article>
      <article><span>닷컴 유사도</span><strong>S1 ${Number(dotcom.scenario_strength?.S1||0).toFixed(2)}</strong><p>승인 계약 ${esc(dotcom.approval_contract_path||'–')} · cap ${Number(dotcom.dependency_cap||0).toFixed(2)}. 0.40은 cap 이내, 0.60은 현재값, 0.80은 cap 초과로 차단합니다.</p></article>
      <article><span>A · B · C 분리</span><strong>A ${Number(weightSpaces.A_evidence_strength?.value||0).toFixed(2)} · B ${Number(weightSpaces.B_generator_dotcom_block_share?.value||0).toFixed(2)} · C ${weightPct(weightSpaces.C_mixture_probability?.value?.S1)}</strong><p>A는 생성 후 S1 증거 강도, B는 S1 생성기의 닷컴 위상 블록 비중, C는 계산 결과인 S1 연구 코호트 질량입니다. C는 직접 입력하지 않습니다.</p></article>
      <article><span>경로 가중치</span><strong>${['S1','S2','S3'].map(key=>`${key} ${weightPct(scenarios[key]?.probability)}`).join(' · ')}</strong><p>서로 다른 역사 DB 군집을 섞는 연구 가중치입니다. 상승·중립·하락의 보정된 발생확률이라는 뜻은 아닙니다.</p></article>
      <article><span>DB 레이어 분리</span><strong>겹친 기준일 0 · 출처 3개</strong><p>완화·균형·긴축 거시 기준일은 생성 전에 상호 배타적으로 나눴고, 세 경로의 블록 출처 해시도 ${num(layerGate.unique_block_provenance_count||0)}개로 독립입니다.</p></article>
      <article><span>PER·밸류에이션</span><strong>수치 반영 보류</strong><p>${valuationGate.status==='REFERENCE_ONLY_MISSING_POINT_IN_TIME_CROSS_ERA_HISTORY'?'당시 공개본 기준의 시대횡단 이력이 없어 참고 정보로만 표시합니다. 오래된 CAPE나 가짜 저PER 값을 대신 넣지 않았습니다.':'검증 상태를 확인해야 합니다.'}</p></article>
      <article><span>발표 반영</span><strong>${num(events.numerical_event_count||0)}건 수치 반영</strong><p>새 CPI·고용·FOMC·GDP·실적은 출처와 발표 시각 검증을 통과한 뒤에만 다음 빌드에 반영됩니다.</p></article>
      <article><span>가산 잔차</span><strong>정의상 0</strong><p>총변화를 구성요소 합으로 정의한 항등 분해입니다. 독립적인 적합도 검증 결과라는 뜻이 아닙니다.</p></article>
    </div></section>
    <section class="scenario-v52-timing"><div class="scenario-v52-section-title"><p class="eyebrow">조정 시점 범위</p><h2>−10%선을 처음 만나는 시기</h2><p>정확한 날짜 예측이 아니라, 조정이 발생한 경로들의 시점 분포입니다.</p></div><div>${(touch.density||[]).map((value,index)=>`<i title="${esc(touch.dates[index])}" style="height:${Math.max(1,Number(value)/Math.max(...(touch.density||[]),1e-12)*100)}%"></i>`).join('')}</div><p>중앙 시점 ${esc(touch.conditional_on_touch_quantiles?.p50||'–')} · 빠른 25% ${esc(touch.conditional_on_touch_quantiles?.p25||'–')} · 늦은 25% ${esc(touch.conditional_on_touch_quantiles?.p75||'–')} · 특정 날짜 확정 아님</p></section>
    <details class="scenario-v52-method"><summary>계산 근거와 한계 자세히 보기</summary><div><article><strong>세 DB는 어떻게 다른가</strong><p>S1은 닷컴 60%와 완화 국면 40%를 acceleration→correction→reacceleration 순환으로 생성합니다. S2는 중립 금리·중간 VIX·완화된 금융여건에서 drift→mean reversion→normalization을, S3는 긴축 국면에서 drawdown→failed relief→stress persistence를 사용합니다.</p></article><article><strong>데이터가 정말 겹치지 않는가</strong><p>완화 ${num(layerGate.macro_regime_cohort_origin_counts?.easing_expansion||0)}개 · 균형 ${num(layerGate.macro_regime_cohort_origin_counts?.balanced_soft_landing||0)}개 · 긴축 ${num(layerGate.macro_regime_cohort_origin_counts?.tightening_stress||0)}개 기준일이며, 쌍별 겹침은 모두 0입니다. 각 생성기는 별도 블록 provenance를 가집니다.</p></article><article><strong>왜 p50은 매끄러운가</strong><p>수천 경로의 날짜별 중앙값이므로 흔들림이 상쇄됩니다. 현실적인 질감은 차트의 가는 점선인 실제 중심 경로로 확인합니다.</p></article><article><strong>승격 4조건</strong><p>직접 사건 ${num(promotionGates.direct_event_observations?.observations||0)}/${num(promotionGates.direct_event_observations?.minimum||60)} · band calibration ${num(promotionGates.band_calibration?.observations||0)}/${num(promotionGates.band_calibration?.minimum||60)} · walk-forward ${esc(promotionGates.walk_forward?.status||'not_approved')} · 승인 run_id ${esc(promotionGates.human_approval?.approval_run_id||'없음')}. champion_eligible=${String(governance.champion_eligible===true)}.</p></article><article><strong>닷컴 강도 감도</strong><p>계산 완료 ${sensitivity.map(row=>`${Number(row.S1_strength).toFixed(2)}→S1 ${weightPct(row.scenario_probabilities?.S1)}`).join(' · ')}. 요청 기준 0.40/0.60은 cap 이내이며 0.80은 사전등록 cap 0.60을 넘어 계산·적용하지 않습니다.</p></article><article><strong>이 전망의 지위</strong><p>${esc(candidate.banner)} · official/champion이 아닌 연구 후보입니다. 모델 식별자 ${esc(candidate.model?.model_id||candidate.candidate_id)}.</p></article><article><strong>재현 식별값</strong><p>${esc(candidate.model_content_sha256)}</p></article></div></details>
  </div>`);
  ...
  if(legacyInsights)legacyInsights.innerHTML=`<div class="scenario-v52-section-title"><p class="eyebrow">숫자의 뜻</p><h2 id="scenario-v52-insights-title">복잡한 지표를 이렇게 읽으세요</h2></div><div>
    <article><span>고용 위험 / 금리 완화</span><strong>${pp(terminalAttr.labor_growth_risk_effect)} / ${pp(terminalAttr.policy_relief_effect)}</strong><p>고용 둔화가 성장 위험을 높이는 효과와 금리 부담 완화 효과를 서로 다른 항으로 계산합니다.</p></article>
    <article><span>닷컴 사이클 강도</span><strong>S1 ${Number(dotcom.scenario_strength?.S1||0).toFixed(2)}</strong><p>닷컴 구간은 상승 시나리오 S1의 등록 에피소드 원천에만 사용합니다. S2와 S3에는 닷컴 강도를 넣지 않습니다.</p></article>
    <article><span>A · B · C 분리</span><strong>A ${Number(weightSpaces.A_evidence_strength?.value||0).toFixed(2)} · B ${Number(weightSpaces.B_generator_dotcom_block_share?.value||0).toFixed(2)} · C ${weightPct(weightSpaces.C_mixture_probability?.value?.S1)}</strong><p>A는 생성 후 S1 증거 강도, B는 S1 생성에 쓰인 등록 닷컴 에피소드 세션 목표 비중(실현 ${weightPct(weightSpaces.B_generator_dotcom_block_share?.realized_session_share)}), C는 계산된 연구 코호트 질량입니다. 서로 같은 확률이 아닙니다.</p></article>
    <article><span>경로 가중치</span><strong>${['S1','S2','S3'].map(key=>`${key} ${weightPct(scenarios[key]?.probability)}`).join(' · ')}</strong><p>각기 다른 에피소드 DB에서 생성된 연구 코호트 질량입니다. 상승·중립·하락의 보정된 발생확률로 해석하지 않습니다.</p></article>
    <article><span>DB 레이어 분리</span><strong>교차 구간 ${num(layerGate.episode_interval_overlap_count||0)} · 잔차 풀 ${num(layerGate.unique_residual_pool_count||0)}개</strong><p>입력 변수 스키마 분리 ${String(layerGate.feature_schemas_distinct===true)} · 잔차 원천 해시 분리 ${String(layerGate.residual_pool_hashes_unique===true)} · 고정 국면 틀 사용 ${String(layerGate.fixed_phase_template_active===true)}.</p></article>
    <article><span>PER · 밸류에이션</span><strong>CAPE · SEC EPS 수치 반영 보류</strong><p>시점별 원본 빈티지가 없어 낡은 CAPE나 다른 SEC 패널을 대용하지 않았습니다. 데이터가 갖춰질 때까지 D0 차단 상태입니다.</p></article>
    <article><span>발표 반영</span><strong>구조 입력 ${num(adapter.source_event_revision_ids?.length||0)}개 · 신규 원장 ${num(events.numerical_event_count||0)}건</strong><p>현재 고용·금리 근거는 에피소드 선택 가중치와 국면 길이에 반영했습니다. 이후 발표는 출처·발표시각·수정관계를 검증해 append-only 원장에 추가한 뒤 재빌드합니다.</p></article>
    <article><span>가산 잔차</span><strong>정의상 0</strong><p>총효과를 구성요소 합으로 정의한 항등 분해이므로 0입니다. 독립적인 적합도 검정이나 별도 예측 성과가 아닙니다.</p></article>
  </div>`;
  const separationInsight=el(`<section class="scenario-v52-insights" aria-labelledby="scenario-v52-separation-title"><div class="scenario-v52-section-title"><p class="eyebrow">세 경로가 달라진 근거</p><h2 id="scenario-v52-separation-title">같은 DB를 색만 바꾼 그래프가 아닙니다</h2></div><div>
    ${['S1','S2','S3'].map(key=>{const row=clusters[key]||{},kernel=row.kernel_audit||{};return `<article><span>${key} · ${esc(V52_SCENARIO_META[key].title)}</span><strong>${num(row.episode_count||0)}개 국면 · 변수 ${num(row.feature_schema?.length||0)}개</strong><p>${key==='S1'?'닷컴·완화·AI 확장 구간':key==='S2'?'비위기 연착륙·횡보 구간':'긴축·금융 스트레스 구간'}만 사용합니다. 고정된 꺾임 날짜 없이 실제 구간 길이와 전환을 다시 뽑습니다. 커널 점검 ${kernel.gate_pass?'통과':'미통과·승격 차단'}.</p></article>`;}).join('')}
    <article><span>경로 유사성</span><strong>${Number(baselineAudit.baseline||0).toFixed(3)} → ${Number(baselineAudit.redesigned_shadow||0).toFixed(3)}</strong><p>S1·S2 p50 로그 경로 상관입니다. 사전 기준보다 실질적으로 낮아졌고, 특정 목표 상관값을 맞추기 위해 선을 변형하지 않았습니다.</p></article>
    <article><span>발표 반영 방식</span><strong>${adapter.structural_update_applied?'구조 선택에 반영':'새 수치 없음'}</strong><p>고용·금리·물가 수치는 확률만 덧칠하지 않고, 어느 과거 구간을 얼마나 선택할지와 국면 길이 선호를 바꿉니다.</p></article>
    <article><span>데이터 한계</span><strong>CAPE·SEC EPS는 아직 보류</strong><p>과거 시점별 원본이 없어 낡은 CAPE나 설비투자 자료를 PER·이익수정 데이터처럼 대신 쓰지 않았습니다.</p></article>
  </div></section>`);
  $('.scenario-v52-timing',outlook)?.before(separationInsight);
  const methodDetails=$('.scenario-v52-method',outlook);
  if(methodDetails)methodDetails.innerHTML=`<summary>계산 근거와 한계 자세히 보기</summary><div>
  ...
  const copies={future:['미래 탐색 · 연구 후보','Scenario V5.2 조건부 경로','서로 다른 데이터 군집의 세 경로를 공통 로그축으로 비교합니다. 보정된 발생확률이나 champion이 아닙니다.'],history:['과거 비교','과거 혁신 사이클은 어떻게 움직였나','과거 사례를 같은 시작점에 맞춰 비교합니다. 현재 전망값이나 사건 확률은 아닙니다.'],'cross-asset':['자산 비교','Bitcoin과 Realty Income은 어떻게 달랐나','NASDAQ과 Realty Income 실측, Bitcoin 민감도 경로를 시작값 100으로 맞춰 비교합니다.'],liquidity:['시장 자금 흐름','유동성이 늘고 줄어든 구간','Fed 순유동성과 NASDAQ·Bitcoin 수익률을 같은 주간축에서 참고용으로 비교합니다.']};
  ...
  labTabs.querySelectorAll('[data-lab-tab]:not(:disabled)').forEach(button=>button.onclick=()=>{activateLab(button.dataset.labTab);history.replaceState(null,'',button.dataset.labTab==='future'?'#future/research':`#future/${button.dataset.labTab}`);});
...
function renderFlow(initialLookup){
  ...
  root.appendChild(el(scenarioModelModeNav('champion',candidate52)));
  ...
  const labTabs=el(`<div class="lab-tabs" role="tablist" aria-label="시장 지도 분석 공간">
    ...
    <button type="button" id="lab-tab-cross-asset" role="tab" aria-selected="false" aria-controls="lab-cross-asset" data-lab-tab="cross-asset" ${crossAsset?'':'disabled'}><span>03</span> 자산 전이<small>${plainTerm('reference_only')}</small></button>
...
function crossAssetPanel(){
  ...
  const w=el(`<div class="chart-panel analysis-panel cross-asset-panel">
    ...
    <div class="panel-head"><div><h2>닷컴 조정 뒤 5년 · NASDAQ · Bitcoin · Realty Income</h2><p>가격 단위가 달라도 방향과 회복 속도를 바로 비교할 수 있게 시작값을 100으로 맞췄습니다.</p></div><span class="count-chip">기준 ${esc(model.asof)}</span></div>
    <section class="plain-insight" aria-label="자산 비교 읽는 법"><article><span>NASDAQ</span><strong>실제 가격</strong><p>2001년 3월 이후 실제 움직임입니다.</p></article><article><span>Realty Income</span><strong>실제 가격·총수익</strong><p>가격과 배당 재투자 효과를 따로 봅니다.</p></article><article><span>Bitcoin</span><strong>가정 경로</strong><p>${esc(model.history.bitcoin?.reason||'2009년 이전 실측이 없어 현대 민감도를 적용한 참고 경로입니다.')}</p></article></section>
    <div class="cross-view-switch" role="group" aria-label="교차자산 보기">
      ...
      <button type="button" data-cross-view="history" aria-pressed="false">NASDAQ·O 실측만</button>
    </div>
    ...
    <section data-cross-panel="history" hidden>
      ...
      <details class="analog-limit annual-return-table"><summary>기준월부터 연차별 실측 수익률 보기</summary><div class="table-shell"><table><thead><tr><th>구간</th><th>NASDAQ 가격</th><th>O 가격</th><th>O 총수익 proxy</th></tr></thead><tbody>${annual.map(row=>`<tr><td><strong>${row.year}년차</strong><small>${esc(String(row.period||'').replace(' to ',' → '))}</small></td><td>${pctText(row.nasdaq_price_pct)}</td><td>${pctText(row.realty_income_price_pct)}</td><td>${pctText(row.realty_income_total_return_pct)}</td></tr>`).join('')}</tbody></table></div></details>
    </section>
...
function crossFiveYearTableMarkup(model,scenario){
  const labels=model.forecast?.labels||[],years=[0,12,24,36,48,60].filter(month=>month<labels.length),paths=scenario.paths||{};
  const rowLabel=month=>month===0?'기준월':`${month/12}년 후`;
  return `<section class="cross-yearly-compare" aria-labelledby="cross-yearly-title"><div class="panel-head"><div><p class="eyebrow">OBSERVED BASELINE · BTC COUNTERFACTUAL</p><h3 id="cross-yearly-title">2001-03부터 2006-03까지 5개년 비교</h3><p>2001-03=100 · NASDAQ·O 두 선은 실측, Bitcoin만 선택 beta에 따른 반사실 값입니다.</p></div><span class="count-chip">${esc(scenario.label)}</span></div>
    <div class="cross-phase-strip"><span><b>OBSERVED</b>NASDAQ · O 가격 · O 총수익 proxy</span><span><b>SYNTHETIC</b>Bitcoin · 2009년 이전 실측 없음</span><span><b>NO PROBABILITY</b>확률·단일 가격 제시·기대수익 아님</span></div>
    <div class="table-shell"><table><thead><tr><th>경과</th><th>NASDAQ 실측</th><th>Bitcoin 반사실</th><th>O 가격 실측</th><th>O 총수익 proxy</th></tr></thead><tbody>${years.map(month=>`<tr><td><strong>${rowLabel(month)}</strong><small>${esc(labels[month])}</small></td>${['nasdaq','bitcoin','realty_income','realty_income_total_return'].map(key=>{const value=Number(paths[key]?.[month]);return `<td><strong style="color:${CROSS_META[key][1]}">${num(value)}</strong><small>${month===0?'비교 기준':signedDelta(value-100,1,' pt')}</small></td>`;}).join('')}</tr>`).join('')}</tbody></table></div>
...
function drawCrossAsset(host,model,scenarioId){
  ...
  drawIndexedCompare(host,{labels:model.forecast.labels,series:scenario.paths,
    bands:scenario.paths_band,keys:['nasdaq','bitcoin','realty_income','realty_income_total_return'],title:`${scenario.label} · 2001-03~2006-03 실측/반사실 비교`,selected:0,history:true,
