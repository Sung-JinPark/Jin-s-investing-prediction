...
function scenarioV5ConditionalFanMarkup(candidate){
  ...
  return `<section class="scenario-v5-fans" aria-labelledby="scenario-v5-fans-title"><div class="scenario-v5-section-head"><div><span>CONDITIONAL DISTRIBUTIONS</span><h3 id="scenario-v5-fans-title">시나리오별 fan과 actual representative</h3></div><small>세 패널 공통 y축 · p10–p90 / p50 / actual member</small></div><div class="scenario-v5-fan-grid">${keys.map(key=>{const row=scenarios[key],p10=pick(row.bands.p10),p90=pick(row.bands.p90),p50=pick(row.bands.p50),member=pick(row.representative_path_values);let band=line(p90);for(let index=p10.length-1;index>=0;index--)band+=` L${X(index).toFixed(1)},${Y(p10[index]).toFixed(1)}`;return `<article><header><div><strong style="color:${esc(row.color)}">${key}</strong><span>${esc(row.label)}</span></div><b>${(Number(row.probability)*100).toFixed(1)}%</b></header><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${key} conditional fan and actual member path"><path d="${band} Z" fill="${esc(row.color)}" opacity=".12"></path><path d="${line(p50)}" fill="none" stroke="#6b7280" stroke-width="1.4" stroke-dasharray="4 3"></path><path d="${line(member)}" fill="none" stroke="${esc(row.color)}" stroke-width="2.4" stroke-linejoin="round"></path></svg><footer><span>p10 ${num(Math.round(p10.at(-1)))}</span><span>p50 ${num(Math.round(p50.at(-1)))}</span><span>p90 ${num(Math.round(p90.at(-1)))}</span><span>ESS ${num(row.weighted_effective_sample_size)}</span></footer></article>`;}).join('')}</div></section>`;
}
