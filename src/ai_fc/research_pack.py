...
PROBABILITY_KEY = re.compile(r"(^|_)(prob|probability|chance|weight)(_|$)", re.I)
...
def _normal(value: Any, key: str = "") -> Any:
    if isinstance(value, dict):
        return {str(k): _normal(v, str(k)) for k, v in value.items()}
    if isinstance(value, list):
        return [_normal(item, key) for item in value]
    if isinstance(value, (int, float)) and not isinstance(value, bool) and PROBABILITY_KEY.search(key):
        return round(float(value) / 100.0, 8) if 1 < float(value) <= 100 else value
    if isinstance(value, str) and PROBABILITY_KEY.search(key):
        try:
            numeric = float(value)
            return round(numeric / 100.0, 8) if 1 < numeric <= 100 else numeric
        except ValueError:
            return value
...
def export_research_pack(root: Path, month: str | None = None) -> Path:
    ...
    if existing.exists():
        ...
        raise ResearchPackError(
            f"immutable research pack already exists with different sources: {target}")
    ...
    (target / "DICTIONARY.md").write_text(
        ...
        "The original normalized row is retained as `payload_json`; `source_file` and "
        ...
        "timestamps are converted to UTC ISO-8601. Missing planned ledgers are intentionally absent.\n",
        encoding="utf-8",
    )
