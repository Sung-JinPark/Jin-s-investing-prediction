...
<section class="panel"><span class="tag">SECONDARY · CONDITIONAL SMALL MULTIPLES</span><h2>S1 / S2 / S3</h2><p>세 선을 main chart에 겹치지 않고 각 조건부 분포를 독립 축으로 표시합니다. 닷컴 유사도 강도는 S1 0.60, S2 0.02, S3 0.03이며 0.60은 research-only override입니다.</p><div class="scenarios">{scenario_cards}</div></section>
...
The owner's 2026-08-10 follow-up explicitly changes the research challenger to
S1/S2/S3 dotcom strengths 0.60/0.02/0.03 and requests the proposed generator:
50% frozen dotcom-neighbor episodes, 30% regime-conditioned bootstrap, and 20%
general historical episodes. The ordinary dependency cap remains 0.35; the
single-cycle dotcom cluster has a separately recorded research-only 0.60
override and remains ineligible for official/champion promotion.
...
""",
        ...
        ) + f"""\n\nFor P(EoY > anchor), labor growth-risk contributes {_pct(attr['terminal_above_anchor_2026']['labor_growth_risk_effect'])}, policy relief {_pct(attr['terminal_above_anchor_2026']['policy_relief_effect'])}, cross/event state {_pct(attr['terminal_above_anchor_2026']['cross_asset_state_effect'])}, and the dotcom S1-weighted view {_pct(attr['terminal_above_anchor_2026']['dotcom_upside_effect'])}. Additivity residual is {attr['terminal_above_anchor_2026']['additivity_residual']:.3g}. S1/S2/S3 dotcom strengths are 0.60/0.02/0.03. The negative one-month analog target remains in the distance function. All ESS, maximum-weight, top-1%, entropy and normalization gates pass.\n""",
        "DOTCOM_AND_EVENT_ADAPTATION.md": f"""# Dotcom weighting and event adaptation\n\nGate: **PASS WITH AGGRESSIVE SINGLE-CYCLE RESEARCH OVERRIDE**\n\nThe registered dotcom kNN case list has one cycle and five dependent neighbors. Its inverse-distance targets are 1m {candidate['dotcom_scenario_weighting']['forward_return_targets']['one_month']:.4f}, 3m {candidate['dotcom_scenario_weighting']['forward_return_targets']['three_month']:.4f}, 6m {candidate['dotcom_scenario_weighting']['forward_return_targets']['six_month']:.4f}, and 12m {candidate['dotcom_scenario_weighting']['forward_return_targets']['twelve_month']:.4f}. S1 receives the explicitly approved research-only strength 0.60; S2 and S3 are reduced to 0.02 and 0.03. The raw generator pool is 50% dotcom / 30% regime / 20% general history. Soft routing leaves the posterior dotcom-engine share at S1 {_pct(candidate['dotcom_scenario_weighting']['path_engine_share_by_scenario']['S1']['dotcom_neighbor_episode_with_local_residual'])}, versus S2 {_pct(candidate['dotcom_scenario_weighting']['path_engine_share_by_scenario']['S2']['dotcom_neighbor_episode_with_local_residual'])} and S3 {_pct(candidate['dotcom_scenario_weighting']['path_engine_share_by_scenario']['S3']['dotcom_neighbor_episode_with_local_residual'])}. S1 probability changes by {_pct(candidate['dotcom_scenario_weighting']['S1_probability_increment'])} against the otherwise-full model. The S1 conditional no-repeat probability moves from {_pct(candidate['dotcom_scenario_weighting']['S1_no_repeat_probability_before_dotcom'])} to {_pct(candidate['dotcom_scenario_weighting']['S1_no_repeat_probability_after_dotcom'])}. No October direction, endpoint, or exact date is forced.\n\nValidated CPI, NFP, FOMC, GDP, and earnings records enter through an append-only JSONL boundary. `available_at <= as_of`, explicit units, source hash, revision id, and `supersedes` are required. CPI/NFP/FOMC/GDP use bounded registered adapters; earnings stays reference-only without an approved asset mapping. A successful CLI ingestion rebuilds and verifies the candidate and refreshes both dashboards. This is explicit event ingestion, not background scraping or unbounded online learning. Active events now: {candidate['event_learning']['active_event_count']}.\n""",
        "PHASE_F_SCENARIO_2027.md": f"""# Phase F — Conditional scenarios and 2027 distinctness\n\nGate: **PASS**\n\nFull-mixture probabilities are S1 {_pct(scenario_probs['S1'])}, S2 {_pct(scenario_probs['S2'])}, S3 {_pct(scenario_probs['S3'])}. These are derived cohort weights, not manually entered probabilities. Every pair differs on at least two registered metrics (2027 terminal return, maximum drawdown, recovery time, annualized volatility).\n\nMain chart semantics: total mixture. Scenario semantics: conditional small multiples. 2027 distinctness gate: `{candidate['distinctness_2027']['gate_pass']}`.\n""",
