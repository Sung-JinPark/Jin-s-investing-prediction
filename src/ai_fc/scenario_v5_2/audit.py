...
from .engine import CANDIDATE_RELATIVE, QUANTILE_NAMES, SOURCE_PATHS
...
<section class="panel"><span class="tag">SECONDARY · CONDITIONAL SMALL MULTIPLES</span><h2>S1 / S2 / S3</h2><p>세 선을 main chart에 겹치지 않고 각 조건부 분포를 독립 축으로 표시합니다. 닷컴 유사도 강도는 S1 0.28, S2 0.04, S3 0.06입니다.</p><div class="scenarios">{scenario_cards}</div></section>
...
def write_reports(root: Path, test_results: str = "not yet supplied") -> list[Path]:
    ...
    reports = {
        ...
        "PHASE_D_HISTORICAL_SHAPE.md": f"""# Phase D — Historical-shape prior\n\nGate: **PASS**\n\nTwo independent deterministic engines use only sessions available by the cutoff: 4,000 stationary-block-bootstrap paths and 4,000 contiguous analog-episode paths. No endpoint, trough date, or scenario probability is forced. The simulation starts after the August 7 close and extends through 2027-12-31.\n\nHistory raw SHA-256: `{candidate['source_hashes'][SOURCE_PATHS[4]]}`. Seed: `{candidate['model']['seed']}`. Main p50 is the pointwise weighted median with no artificial wiggle. Seven actual members plus an actual dotted medoid carry path texture.\n""",
...
def build_review_package(root: Path) -> tuple[Path, str]:
    ...
    for pattern in (
        ...
        "data/scenarios/candidates/scenario_v5_2_macro_actualized_historical_shape_v1_latest.json",
