...
<p>{html.escape(payload['anchor']['date'])} 최신 완료 시장일 {payload['anchor']['close']:,.2f}에서 시작하는 research candidate입니다. official·champion이 아닙니다.</p>
...
<section class="panel"><span class="tag">AUDIT DETAIL · CONDITIONAL DISTRIBUTIONS</span><h2>서로 겹치지 않는 DB에서 생성한 S1 / S2 / S3</h2><p>고객 화면은 세 p50을 한 로그축에 통합합니다. 이 감사 화면은 분포 폭을 확인하기 위해 조건부 패널도 함께 둡니다. S1은 닷컴 60%+완화 거시 40%, S2는 균형·연착륙 거시, S3는 긴축·스트레스 거시 원점과 독립 phase sampler를 사용합니다. 거시 기준일 쌍별 겹침은 0입니다.</p><div class="scenarios">{scenario_cards}</div></section>
...
def write_reports(root: Path, test_results: str = "not yet supplied") -> list[Path]:
    ...
    calibration_observations = max(
        0, len(calibration_path.read_text(encoding="utf-8").splitlines()) - 1
    ) if calibration_path.is_file() else 0
...
""",
        "PHASE_B_DATA_PROVENANCE.md": f"""# Phase B — Macro and market data provenance\n\nGate: **PASS WITH SOURCE-APPROVAL BLOCKS**\n\n- BLS release `{labor['release_id']}`: payroll {labor['actual']['nonfarm_payroll_change']:,}, unemployment {_pct(labor['actual']['unemployment_rate'])}, participation {_pct(labor['actual']['labor_force_participation_rate'])}.\n- May/June revisions: {labor['revisions'][0]['revision']:,} / {labor['revisions'][1]['revision']:,}; combined {labor['combined_revision']:,}.\n- The Kiplinger payroll consensus is narrative-only and is excluded from the numerical labor score.\n- Every normalized rate distribution has explicit unit `fraction` and sums to one, but the Investing.com-derived distribution is reference-only unless an authoritative archived-source approval receipt passes the source gate. Current status: `{candidate['numerical_source_governance']['fed_rate_distribution']['status']}`.\n- Aggregate reference values: Sep {_pct(rates['aggregate_hike_probability']['2026-09-30']['pre'])} → {_pct(rates['aggregate_hike_probability']['2026-09-30']['post'])}; Oct {_pct(rates['aggregate_hike_probability']['2026-10-28']['pre'])} → {_pct(rates['aggregate_hike_probability']['2026-10-28']['post'])}; Dec {_pct(rates['aggregate_hike_probability']['2026-12-09']['pre'])} → {_pct(rates['aggregate_hike_probability']['2026-12-09']['post'])}.\n- AP/Investing cross-asset observations are also reference-only without the same class of authoritative receipt. Current status: `{candidate['numerical_source_governance']['post_jobs_cross_asset_state']['status']}`.\n- Yahoo ^IXIC PIT history: 2,664 closes, 2016-01-04 through 2026-08-07; raw hash `{candidate['source_hashes'][SOURCE_PATHS[4]]}`.\n""",
        "PHASE_C_EVENT_KERNEL.md": f"""# Phase C — Labor vector and event kernel\n\nGate: **PASS WITH LIMITATION**\n\nThe numerical labor vector uses the authoritative BLS payroll level, combined revision, and temporary layoffs. The private consensus field is ignored. Missing fields are rejected rather than filled with zero. Growth-risk score is `{candidate['evidence_scores']['labor_growth_risk']['bounded_score']:.6f}`. Policy-relief remains a separate latent factor, `{candidate['evidence_scores']['policy_relief']['bounded_score']:.6f}`, and its pre/post rate component is zero unless the source-approval gate passes.\n\nOnly one eligible event is available for a direct historical event-return map. Under the n<30 rule, the hard event kernel is `REFERENCE_ONLY_INSUFFICIENT_N`; it contributes no direct price jump. The August 7 Nasdaq return is already in the {candidate['anchor']['close']:,.2f} anchor. Future event jump = 0, event-return coefficient = 0, equality-with-zero-event-reaction gate = `{candidate['circularity_control']['full_equals_explicit_zero_event_reaction']}`.\n""",
        "PHASE_D_HISTORICAL_SHAPE.md": f"""# Phase D — Independent multilayer database paths\n\nGate: **PASS**\n\nEach scenario has 3,000 paths from a different historical regime layer and independent block provenance. The macro history is partitioned before clustering into easing {candidate['model']['generator_audit']['macro_regime_cohort_origin_counts']['easing_expansion']}, balanced {candidate['model']['generator_audit']['macro_regime_cohort_origin_counts']['balanced_soft_landing']}, and tightening {candidate['model']['generator_audit']['macro_regime_cohort_origin_counts']['tightening_stress']} origin dates; every pairwise overlap count is zero. S1 uses dotcom B=0.60 plus easing-macro B=0.40 in acceleration/correction/reacceleration phases. S2 uses balanced drift/mean-reversion/normalization phases. S3 uses tightening drawdown/failed-relief/stress-persistence phases. All use full realized historical blocks at scale 1.00. Deterministic k-medoids sees only information available at each origin. Forward returns and drawdowns are read only after assignments freeze and label whole clusters; no individual origin is selected by its forward result.\n\nSelected 252-session median returns are S1 {clusters['S1']['selected_cluster']['outcome_medians']['forward_return_252d']:.4f}, S2 {clusters['S2']['selected_cluster']['outcome_medians']['forward_return_252d']:.4f}, and S3 {clusters['S3']['selected_cluster']['outcome_medians']['forward_return_252d']:.4f}. Selected origin counts are {clusters['S1']['selected_cluster']['origin_count']}/{clusters['S2']['selected_cluster']['origin_count']}/{clusters['S3']['selected_cluster']['origin_count']}. All three provenance hashes are unique. No endpoint or exact turning date is forced. PER/valuation is reference-only because a vintage-complete cross-era PIT history is unavailable.\n\nGeneral-history raw SHA-256: `{candidate['source_hashes'][SOURCE_PATHS[4]]}`. Dotcom daily raw SHA-256: `{candidate['source_hashes'][SOURCE_PATHS[11]]}`. Macro-cluster raw SHA-256: `{candidate['source_hashes'][SOURCE_PATHS[13]]}`. Seed: `{candidate['model']['seed']}`. p50 is an unmodified pointwise weighted median; actual medoids carry path texture.\n""",
...
def build_review_package(root: Path) -> tuple[Path, str]:
    ...
    for pattern in (
        ...
        ".github/workflows/ai-regime-refresh.yml",
