    ...
    def _build_prompt(envelope: dict[str, Any]) -> str:
        ...
        if envelope.get("task_key") == "R4-S4-006":
            qualification_instruction += (
                ...
                "R4-S4-005 in mechanism_artifacts as a named JSON object keyed by task ID; each value "
                "must be an object with keys path and sha256 (not acceptance_summary_sha256). Emit a "
                "complete G3 score matrix at the declared "
                ...
                "Put those comparator, horizons, weekly_origins=true, origin_count, score_rows and "
                "evaluation_coordinate_grid_hash fields inside a frozen_evaluation object. For every "
                "stress component emit entries in an array named components (not stress_components), "
                "with component_id, oos_stacking_advantage, admitted and "
                ...
                "and predecessor rows contribute zero selection or tuning rows using the exact keys "
                "legacy_review_pack_score_rows_used, qualification_score_rows_used and outer_rows_used. "
                "Preserve R4-M3-008 qualification revision 3 and score matrix SHA-256 inside a sealed_prior "
                "object with qualification_revision_3_sha256 and score_matrix_sha256 "
        ...
        return (
            ...
            "failure as an unexplained failed command. Mark an intentional environment probe that "
            "fails before a later successful command as phase=expected_diagnostic with a non-empty "
            "expected_failure; do not hide it or call it a successful command. The only success "
            "status literal is SUCCEEDED; "
            ...
            "For a SUCCEEDED result, the tests array contains final validation outcomes only and all "
