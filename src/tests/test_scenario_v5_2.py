...
def test_structured_employment_consensus_is_used_and_disclosed_honestly() -> None:
    payload = _candidate()
    labor = load_inputs(ROOT)["labor"]
    expected_surprise = (
        labor["actual"]["nonfarm_payroll_change"]
        - labor["consensus"]["nonfarm_payroll_change"]
    ) / 100000.0
    assert math.isclose(
        payload["evidence_scores"]["labor_growth_risk"]["components"][
            "payroll_surprise_z"
        ],
        expected_surprise,
    )
    consensus = next(
        row for row in payload["evidence_registry"]
        if row["evidence_id"] == "kiplinger_jobs_consensus_and_commentary"
    )
    assert consensus["used_numerically"] is True
    assert consensus["effective_strength"] == 0.0
    assert consensus["dependency_cluster_id"] == "bls_empsit_2026_07"
    assert "no_extra_strength" in consensus["role"]
    assert payload["circularity_control"][
        "structured_forecast_consensus_used_numerically"
    ] is True
    assert payload["circularity_control"][
        "narrative_reports_used_numerically"
    ] is False
